...
RegisterServerEvent('qb-vehicleshop:server:buy-stock', function(vehicle)
    ...
    if Player.PlayerData.money['bank'] >= vehiclePrice + discount then
        TriggerClientEvent("qb-vehicleshop:client:create_blip", src, vehicle)
        Player.Functions.RemoveMoney('bank', vehiclePrice + discount)
        TriggerClientEvent('QBCore:Notify', src, 'Stok alındı ve konum haritada işaretlendi.', "success")
    else
        TriggerClientEvent('QBCore:Notify', src, 'Stok satın almak için yeterli paraya sahip değilsin.', "error")
    end
end)
...
RegisterNetEvent('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    ...
    if cash > tonumber(vehiclePrice) then
        MySQL.insert('INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, garage, state, ikinciel) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', {
        ...
        TriggerEvent('DiscordBot:ToDiscord', 'aracalma', vehicle..' - ' ..plate.. ' Plakalı Aracı '..vehiclePrice..' Fiyatına Satın Aldı', src)
...
QBCore.Commands.Add('aracıdevret', Lang:t('general.command_transfervehicle'), { { name = 'ID', help = Lang:t('general.command_transfervehicle_help') }, { name = 'amount', help = Lang:t('general.command_transfervehicle_amount') } }, false, function(source, args)
    local src = source
    local buyerId = exports.mUID:GetIDfromUID(tonumber(args[1]))
